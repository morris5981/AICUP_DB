...
class Trainer:
    ...
    def train(self):
        ...
        while True:
            ...
            print('Epoch: {}/{} --- < Starting Time : {} >'.format(epoch,self.experiment.train.epochs,localtime))
            print('-' * len('Epoch: {}/{} --- < Starting Time : {} >'.format(epoch,self.experiment.train.epochs,localtime)))
            ...
            for batch in train_data_loader:
                batch_item += 1
                localtime = time.asctime( time.localtime(time.time()) )
                print('Batch: {}/{} --- < Starting Time : {} >'.format(batch_item,self.total,localtime))
            ...
            if epoch == 10:
                self.model_saver.save_checkpoint(model, 'AICUP_final_epochs10.pth')
            if epoch == 15:
                self.model_saver.save_checkpoint(model, 'AICUP_final_epochs15.pth')
            if epoch == 20:
                self.model_saver.save_checkpoint(model, 'AICUP_final_epochs20.pth')
            if epoch == 25:
                self.model_saver.save_checkpoint(model, 'AICUP_final_epochs25.pth')

            epoch += 1
            
            if epoch > self.experiment.train.epochs:
                self.model_saver.save_checkpoint(model, 'AICUP_final.pth')
                if self.experiment.validation:
                    self.validate(validation_loaders, model, epoch, self.steps)
