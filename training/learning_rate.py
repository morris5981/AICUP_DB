...
class CosineLearningRate(Configurable):
    lr = State(default=0.007)
    epochs = State(default=1200)
    factor = State(default=0.9)
    cycle_step = State(default=5)

    def __init__(self, **kwargs):
        self.load_all(**kwargs)

    def get_learning_rate(self, epoch, step=None):
        # rate = math.cos(0.5 * epoch / self.epochs * math.pi)
        rate = math.cos(0.5 * (epoch % self.cycle_step) / self.epochs * math.pi)
        return rate * self.lr
